...
class options():
    ...
    Name = 'BF ZIP'
    Call = 'bfzip'
    Modulename = 'bruteforcezip'
    Category = 'bruteforce'
    Type = 'zip'  # sin = single action/program, aut = multiple programs combined for attack
    Version = '0.1'
    License = 'MIT'
    Description = 'Bruteforce a ZIP file'
...
def extract_file(passwords):
    global waitpwd
    with zipfile.ZipFile(sop.file) as zipf:
        for password in passwords:
            try:
                zipf.extractall(path='tmp/', pwd=password)
...
# CONSOLE
def console():
    value = input("   -> " + bc.FAIL + "wmd" + bc.ENDC + "@" + bc.FAIL + "bfzip:" + bc.ENDC + " ")
    ...
    if 'so' in userinput[:1]:
        ...
        run()
        input("   -> " + bc.FAIL + "wmd" + bc.ENDC + "@" + bc.FAIL + "bfzip:" + bc.ENDC + " [*] Please wait while bruteforcing with " + sop.threads + " threads")
...
def main():
    print('\n')
    print('     ___  ____  ____  _______   ')
    print('    / _ )/ __/ /_  / /  _/ _ \  ')
    print('   / _  / _/    / /__/ // ___/  ')
    print('  /____/_/     /___/___/_/      ')
    print('')
    print('\n')
    global sop
